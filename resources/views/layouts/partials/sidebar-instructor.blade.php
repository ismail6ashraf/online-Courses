...
<a class="nav-link {{ request()->routeIs('instructor.tasks.*') ? 'active' : '' }}"
    ...
    @php $c = \App\Models\InstructorTask::where('instructor_id', auth()->id())->where('status', 'pending')->count(); @endphp
    @if($c > 0)<span class="badge bg-warning text-dark rounded-pill ms-auto">{{ $c }}</span>@endif
</a>
