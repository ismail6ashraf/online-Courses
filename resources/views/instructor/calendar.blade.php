...
@section('title', 'Teaching Calendar')
@section('page-title', 'Teaching Calendar')

@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-semibold mb-1">Teaching Calendar</h4>
            <p class="text-muted small mb-0">Sessions and assignment deadlines for {{ $month->format('F Y') }}.</p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('instructor.calendar', ['month' => $month->copy()->subMonth()->format('Y-m')]) }}" class="btn btn-sm btn-outline-secondary">
                <i class="bi bi-chevron-left"></i>
            </a>
            <a href="{{ route('instructor.calendar') }}" class="btn btn-sm btn-outline-primary">Today</a>
            <a href="{{ route('instructor.calendar', ['month' => $month->copy()->addMonth()->format('Y-m')]) }}" class="btn btn-sm btn-outline-secondary">
                    ...
                    <div class="d-flex flex-column gap-1">
                        @foreach($dayEvents as $event)
                            <a href="{{ $event['url'] }}" class="calendar-event text-decoration-none {{ $event['type'] === 'session' ? 'event-session' : 'event-assignment' }}">
                                ...
                                <span class="d-block text-truncate opacity-75">{{ $event['subtitle'] }}</span>
                            </a>
                        @endforeach
                    </div>
                </div>
