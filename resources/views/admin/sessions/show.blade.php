...
@section('content')
    ...
    <div class="row g-3 mb-3">
        <div class="col-sm-3">
            <div class="card text-center p-3">
                <div class="fs-3 fw-bold text-primary">{{ $session->attendances->count() }}</div><small
                    class="text-muted">Attendees</small>
            </div>
        </div>
        <div class="col-sm-3">
            <div class="card text-center p-3">
                <div class="fs-3 fw-bold text-danger">
                    {{ $session->behaviorIncidents->where('type', 'negative_speech')->count() }}</div><small
                    class="text-muted">Negative Speech</small>
            </div>
        </div>
        <div class="col-sm-3">
            <div class="card text-center p-3">
        ...
        <div class="col-sm-3">
            <div class="card text-center p-3">
                <div class="fs-3 fw-bold text-danger">{{ $session->dataLeakageIncidents->count() }}</div><small
                    class="text-muted">Data Leakage</small>
            </div>
        </div>
    </div>
    ...
    <div class="row g-3">
                ...
                <div class="list-group list-group-flush">
                    @forelse($session->attendances as $att)
                        <div class="list-group-item d-flex justify-content-between">
                            ...
                            <span
                                class="badge bg-{{ $att->status === 'present' ? 'success' : 'danger' }}">{{ ucfirst($att->status) }}</span>
                        </div>
                    @empty
        ...
        <div class="col-lg-6">
            <div class="card">
                <div class="card-header bg-transparent">
                    <h6 class="mb-0">Behavior Incidents</h6>
                </div>
                <div class="list-group list-group-flush">
                    @forelse($session->behaviorIncidents as $inc)
                        <div class="list-group-item">
                            <div class="d-flex gap-2">
                                <span
                                    class="badge bg-{{ $inc->type === 'negative_speech' ? 'danger' : 'success' }}">{{ $inc->type === 'negative_speech' ? 'NEG' : 'POS' }}</span>
                                <div>
